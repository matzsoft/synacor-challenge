...
When you sign up on the website you receive a download containing the architectural description of a virtual machine and a binary file with the initial memory of a program that runs on that machine.  The description file also contains some hints and the first of eight codes to be enterred into the website to record your progress.

Implementing the virtual machine was very straight forward so I decided to complete that first.  But one of the hints was that you only needed to implement 3 intsructions to get started.  So I did that and was rewarded with a welcome message and the second of the eight codes. The welcome message gives the next hint, the virtual machine must pass a self-test. So I implemented the remaining instructions.  Now running again the self-test passes, a third code is revealed, and you enter the beginning of text based adventure game.  There are obvious references to Advent, Zork, and the many that followed.

## Let There be Light

The beginning of the game is quite simple.  I saw no need to draw a map until I had discoverd an empty lantern, an area of deadly Darkness, and a maze of Twisty Passges at the bottom of a ladder. The passages are not quite as all alike as they first seem so I was able to map the maze without resorting to dropping items.  I soon discoverd two more entrances to the Darkness.  At this point I was unsure if all three areas of Darkness were the same or different.
...
Once I fully mapped the maze I found the can of oil and that allowed me to light the lantern.  I then found that the two areas of Darkness at the bottom of the ladder were the same and unintersting.  Now it was time to go back up the ladder.
...
Solving the puzzle openned a door leading to the next part of the challenge.
...
So I reimplemented the function without recursion.  Again this worked fine at `r7 == 1` but was still running for `r7 == 2` when I noticed that my memory footprint was up to 30GB.  After a lot of head scratching I realized that I had missed the second twist.
...
Knowing the author's fondness for the Manhatten Distance I mentally checked all 20 paths with the minimum distance with no success.  So I decided to write code to find the shortest path.  A simple breadth first search did the trick.

So I impemented a `solve vault` command that takes the orb, calculates and navigates the correct path.  Running this command from the Vault Antechamber leaves at the unlocked vault door.

Entering the vault reveals untold riches and a mirror.  Using the mirror reveals the final code written on your forehead. But when I entered this code it was rejected. I racked my brain trying to figure out what I could have done wrong until I realized it was a mirror.  By design all the letters in the code were either symetrical or transformed to another valid letter in the mirror.  Once I flipped the code I was done.